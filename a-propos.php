<!doctype html>
<html lang="en">

<?php     
        include 'components/head.php'
    ?>

<body>

  <div id="pageloader">
    <div class="loader-item">
      <div class="loader">
        <div class="circle"></div>
        <div class="circle"></div>
        <div class="circle"></div>
        <div class="circle"></div>
      </div>
    </div>
  </div>

  <?php     
        include 'components/inner_nav.php'
    ?>

  <section class="breadcrumbs-page-wrap">
    <div class="bg-fixed pos-rel breadcrumbs-page">
      <div class="container">
        <h1>Qui sommes nous</h1>
        <nav aria-label="breadcrumb" class="breadcrumb-wrap">
          <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="index.php">Acceuil</a></li>
            <li class="breadcrumb-item active" aria-current="page">A Propos</li>
          </ol>
        </nav>
      </div>
    </div>
  </section>


  <main id="body-content">

    <section class="wide-tb-100">
      <div class="container">
        <div class="row align-items-center">
          <div class="col-lg-7 col-md-12">
            <div class="text-center">
              <img src="assets/images/about.png" alt="">
            </div>
          </div>
          <div class="col-lg-5 col-md-12">
            <h1 class="heading-main">
              <small>Nous connaître</small>
              UVUE-Togo, pour  un monde socialement plus juste et plus humain.
            </h1>
            <p>« Une Vie Un Espoir » est une association de jeunes dynamiques, Togolais et non-togolais pluriels par leur diversité culturelle, venant des quatre coins de la planète. Dévouée à la cause du bien-être de la population en vue d’un développement durable, la philosophie de UVUE –Togo est le don de soi pour les autres</p>
           
            
          </div>
        </div>
      </div>
    </section>


    <section class="wide-tb-100 bg-white mb-spacer-md">
      <div class="container">
        <div class="row">
          <div class="col-lg-5 col-md-12">
            <h1 class="heading-main">
              <small>Notre esprit</small>
             Le coeur de nos actions
            </h1>
            <p>UVUE-Togo est sensible à l’encouragement des rencontres entre les bénévoles locaux, et ceux venus d’horizons différents.
            </p>

              <p>Nous encourageons la solidarité internationale, l’échange et le partage des connaissances et savoirs. La santé des populations et l’épanouissement de la jeune fille et de la femme occupent une place de choix dans nos programmes. 
              </p>

            

          </div>

          <div class="w-100 d-none d-sm-block d-lg-none spacer-60"></div>

          <div class="col-lg-7 col-md-12">

            <ul class="nav nav-pills theme-tabbing mb-3" id="pills-tab" role="tablist">
              <li class="nav-item">
                <a class="nav-link active" id="pills-mission-tab" data-toggle="pill" href="#pills-mission" role="tab"
                  aria-controls="pills-mission" aria-selected="true">Mission</a>
              </li>
              <li class="nav-item">
                <a class="nav-link" id="pills-vision-tab" data-toggle="pill" href="#pills-vision" role="tab"
                  aria-controls="pills-vision" aria-selected="false">principes d'action</a>
              </li>
            </ul>
            <div class="tab-content theme-tabbing" id="pills-tabContent">
              <div class="tab-pane fade show active" id="pills-mission" role="tabpanel"
                aria-labelledby="pills-mission-tab">
                <div class="row align-items-center">
                  <div class="col-md-7">
                    <div class="about-img-small">
                      <img src="assets/images/about_img_2.jpg" class="about-us-2" alt="">
                      
                    </div>
                  </div>
                  <div class="col-md-5">
                    <p> UVUE-Togo a pour mission d’aider les populations rurales à atteindre le bien-être social et culturel en favorisant le développement participatif et les échanges inter-communautés.</p>
                  </div>
                </div>
              </div>
              <div class="tab-pane fade" id="pills-vision" role="tabpanel" aria-labelledby="pills-vision-tab">
                <div class="row align-items-center">
                  <div class="col-md-7">
                    <div class="about-img-small">
                      <img src="assets/images/about_img_2.png" class="about-us-2" alt="">
                      
                    </div>
                  </div>
                  <div class="col-md-5">
                    <p>Les principes qui fondent et orientent les actions de UVUE-Togo sont le volontariat, le bénévolat et la disponibilité. Pour mettre en avant les échanges culturels, UVUE-Togo s’attache donc à faire venir les étrangers soucieux de vivre une expérience dans notre région (Afrique de l’Ouest)
                    </p>
                    </ul>
                  </div>
                </div>
              </div>
             
            </div>

          </div>
        </div>
      </div>
    </section>


    <section class="wide-tb-100 team-bg mb-spacer-md">
      <div class="container">
        <div class="row justify-content-between align-items-end">
          <div class="col-lg-4 col-md-6">
            <h1 class="heading-main">
              <small>L'équipe</small>
              Notre équipe & volontaire
            </h1>
          </div>
          <div class="col-lg-8 col-md-6 text-md-right btn-team">
            <a href="devenir-benevole.php" class="btn btn-outline-dark">Nous rejoindre</a>
          </div>
        </div>
        <div class="row">

          <div class="col-12 col-lg-3 col-sm-6">
            <div class="team-section-wrap mb-0">
              <div class="img green">
                
                <img src="assets/images/team/team-1.jpg" alt="" class="rounded-circle">
              </div>
              <h4>Mississo Koami Edoh</h4>
              <h5>Présient</h5>
            </div>
          </div>


          <div class="col-12 col-lg-3 col-sm-6">
            <div class="team-section-wrap mb-0">
              <div class="img navy-blue">
                
                <img src="assets/images/team/team-2.jpg" alt="" class="rounded-circle">
              </div>
              <h4>Assogba Koffi Bruno  Mawutoe</h4>
              <h5>Sécrétaire</h5>
             
            </div>
          </div>


          <div class="w-100 d-none d-sm-block d-lg-none spacer-60"></div>


          <div class="col-12 col-lg-3 col-sm-6">
            <div class="team-section-wrap mb-0">
              <div class="img orange">
                
                <img src="assets/images/team/team-3.jpg" alt="" class="rounded-circle">
              </div>
              <h4>Assogba Komi Freeman</h4>
              <h5>Tréosrier</h5>
            </div>
          </div>


          <div class="col-12 col-lg-3 col-sm-6">
            <div class="team-section-wrap mb-0">
              <div class="img beige">
               
                <img src="assets/images/team/team-4.jpg" alt="" class="rounded-circle">
              </div>
              <h4>Rick Delaf Kossi Ouwolowoudou Jacques</h4>
              <h5>Chargé de la communication</h5>
            </div>
          </div>

        </div>
      </div>
    </section>


    




    <section class="wide-tb-150 bg-scroll bg-help-us pos-rel callout-style-1">
      <div class="bg-overlay blue opacity-80"></div>
      <div class="container">
        <div class="row align-items-center">
          <div class="col-lg-7">
            <h1 class="heading-main light-mode">
              <small>Tous pour un monde meilleur</small>

                  Promouvoir un mode de vie meilleur basé sur la solidarité mutuelle
            </h1>
          </div>
          
        </div>
      </div>
    </section>



    <section class="wide-tb-100">
      <div class="container">
        <div class="row justify-content-between align-items-end">
          <div class="col-lg-6 col-md-8">
            <h1 class="heading-main">
              <small>Nous rejoindre</small>
              Joignez vous à nos activités
            </h1>
          </div>
        </div>
        <div class="owl-carousel owl-theme" id="home-second-events">

          <div class="item">
            <div class="event-wrap-alternate">

              <div class="img-wrap">
               
                <a href=""><img src="assets/images/events/event_alternate_img_1.jpg" alt=""></a>
                <div class="content-wrap">
                  <h3><a href="devenir-benevole.php">VOYAGER AVEC NOUS</a></h3>
                  <div class="event-details">
                    <a href="devenir-benevole.php" class="btn btn-default">En savoir plus</a>
                  </div>
                </div>
              </div>

            </div>
          </div>


          <div class="item">
            <div class="event-wrap-alternate">

              <div class="img-wrap">
               
                <a href="devenir-benevole.php"><img src="assets/images/events/event_alternate_img_3.jpg" alt=""></a>
                <div class="content-wrap">
                  <h3><a href="devenir-benevole.php">STAGE INTERNATIONAL</a></h3>
                  <div class="event-details">
                    <a href="devenir-benevole.php" class="btn btn-default">En savoir plus</a>
                  </div>
                </div>
              </div>

            </div>
          </div>


          <div class="item">
            <div class="event-wrap-alternate">

              <div class="img-wrap">
               
                <a href="devenir-benevole.php"><img src="assets/images/events/event_alternate_img_4.jpg" alt=""></a>
                <div class="content-wrap">
                  <h3><a href="devenir-benevole.php">STAGE INFIRMIER​  </a></h3>
                  <div class="event-details">
                    <a href="devenir-benevole.php" class="btn btn-default">En savoir plus</a>
                  </div>
                </div>
              </div>

            </div>
          </div>


          <div class="item">
            <div class="event-wrap-alternate">

              <div class="img-wrap">
               
                <a href="devenir-benevole.php"><img src="assets/images/events/event_alternate_img_5.jpg" alt=""></a>
                <div class="content-wrap">
                  <h3><a href="devenir-benevole.php">SOUTIEN SCOLAIRE</a></h3>
                  <div class="event-details">
                    <a href="devenir-benevole.php" class="btn btn-default">En savoir plus</a>
                  </div>
                </div>
              </div>

            </div>
          </div>


          <div class="item">
            <div class="event-wrap-alternate">

              <div class="img-wrap">
                <a href="devenir-benevole.php"><img src="assets/images/events/event_alternate_img_6.jpg" alt=""></a>
                <div class="content-wrap">
                  <h3><a href="devenir-benevole.php">BÉNÉVOLAT - RENFORCEMENT ASSOCIATIF                  </a></h3>
                  <div class="event-details">
                    <a href="devenir-benevole.php" class="btn btn-default">En savoir plus</a>
                  </div>
                </div>
              </div>

            </div>
          </div>

          <div class="item">
            <div class="event-wrap-alternate">

              <div class="img-wrap">
               
                <a href="devenir-benevole.php"><img src="assets/images/events/event_alternate_img_7.jpg" alt=""></a>
                <div class="content-wrap">
                  <h3><a href="devenir-benevole.php">CAMPS CHANTIERS                  </a></h3>
                  <div class="event-details">
                    <a href="devenir-benevole.php" class="btn btn-default">En savoir plus</a>
                  </div>
                </div>
              </div>

            </div>
          </div>

        </div>
        
      </div>
    </section>




  </main>
  <?php     
        include 'components/footer.php';
        include 'components/back-to-top.php';
        include 'components/footer-script.php'
    ?>
</body>


</html>